...
async function fetchHistoricalCandlesFromDataset(datasetId: string): Promise<MarketData[]> {
  console.log(`📁 Loading REAL historical data from local dataset: ${datasetId}`);
  ...
  try {
    ...
    console.log(`✅ Loaded ${data.length} REAL candles from local dataset (UNLIMITED)`);
    return data;
...
export class BacktestEngine {
  ...
  async runBacktest(params: BacktestParams): Promise<BacktestResults> {
    ...
    console.log(`📡 Loading REAL historical data from dataset (UNLIMITED & FREE)`);
    
    // Load real historical data from local dataset
    const historicalData = await fetchHistoricalCandlesFromDataset(params.datasetId);
    ...
    for (let i = 20; i < historicalData.length; i++) { // Start at 20 for indicators
      ...
      if (openTrades.length < 3) {
        ...
        if (params.strategy === 'dual_timeframe_trend' && htfData) {
          ...
          signal = this.trendStrategy.analyzeMarket(currentData, htfWindow, indicators, htfIndicators);
        } else {
          signal = this.getStrategySignal(params.strategy, currentData, indicators);
        }
        ...
        if (signal.confidence >= 75 && (signal.action === 'buy' || signal.action === 'sell')) {
          ...
          const riskCheck = this.riskManager.checkRiskLimits(
            ...
            0, // Daily loss not tracked in backtest
            currentBalance,
            params.propFirm
          );
          ...
          if (riskCheck.canTrade) {
            ...
            if (newTrade) {
              ...
              console.log(`📈 New ${newTrade.action.toUpperCase()} trade: ${newTrade.entryPrice} | SL: ${newTrade.stopLoss} | TP: ${newTrade.takeProfit}`);
            }
          }
        }
      }
      ...
      // Update equity curve
      const runningPnL = closedTrades.reduce((sum, trade) => sum + (trade.pnl || 0), 0);
    ...
    for (const trade of openTrades) {
      ...
      trade.duration = new Date(trade.exitTime).getTime() - new Date(trade.entryTime).getTime();
    ...
    console.log(`📊 Results: ${results.totalTrades} trades, ${results.winRate.toFixed(1)}% win rate, ${results.totalReturnPercent.toFixed(2)}% return`);
    
    return results;
  }

  // Determine a sensible HTF for a given LTF
  private getDefaultHTF(lt: BacktestParams['timeframe']): BacktestParams['timeframe'] {
    const map: Record<BacktestParams['timeframe'], BacktestParams['timeframe']> = {
      '1m': '15m',
      '5m': '30m',
      '15m': '1h',
      '30m': '4h',
  ...
  private symbolSpecCache: { minVolume: number; volumeStep: number } | null = null;
  ...
  private calculateTechnicalIndicators(data: MarketData[]): TechnicalIndicators {
    ...
    return {
      ...
      sessionHigh,
      sessionLow,
      isHighVolatility: atr > this.calculateSMA(data.slice(-20).map((d, i, arr) => 
  ...
  private checkTradeExits(
  ...
  ): { updatedTrades: BacktestTrade[]; closedTradesThisPeriod: BacktestTrade[] } {
    ...
    for (const trade of openTrades) {
      ...
      if (trade.action === 'buy') {
        ...
        trade.maxFavorableExcursion = Math.max(trade.maxFavorableExcursion || 0, currentProfit);
        trade.maxAdverseExcursion = Math.min(trade.maxAdverseExcursion || 0, currentLoss);
  ...
  private getStrategySignal(strategy: string, data: MarketData[], indicators: TechnicalIndicators): TradingSignal {
    switch (strategy) {
      case 'vab_breakout':
  ...
  private createTradeFromSignal(signal: TradingSignal, candle: MarketData, balance: number, riskPercent: number): BacktestTrade | null {
    const stopLossDistance = Math.abs(candle.close - signal.stopLoss);
    const riskAmount = balance * (riskPercent / 100);
    let volume = riskAmount / (stopLossDistance || 1e-9);

    // Apply symbol specification constraints (minVolume, volumeStep)
  ...
  private calculateBacktestMetrics(
    ...
    return {
      strategy: params.strategy.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()),
      ...
      maxDrawdownPercent: Math.round((maxDrawdown / params.initialBalance) * 10000) / 100,
      ...
      averageTradeDuration: trades.length > 0 
        ? trades.reduce((sum, t) => sum + (t.duration || 0), 0) / trades.length / (1000 * 60) // Convert to minutes
        : 0,
      ...
      averageMFE: trades.length > 0 
        ? trades.reduce((sum, t) => sum + (t.maxFavorableExcursion || 0), 0) / trades.length
        : 0,
      averageMAE: trades.length > 0 
        ? trades.reduce((sum, t) => sum + (t.maxAdverseExcursion || 0), 0) / trades.length
        : 0,
      trades,
      equityData,
      executionTime,
  ...
  private calculateTrueRange(current: MarketData, previous?: MarketData): number {
    if (!previous) {
